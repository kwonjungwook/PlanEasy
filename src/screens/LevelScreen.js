...
const LevelScreen = ({ navigation }) => {
  ...
  const getNextLevelBadges = () => {
    return ALL_BADGES.filter(
      (badge) => badge.level === level + 1 && !earnedBadges.includes(badge.id)
    );
  };
    ...
    <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
      ...
      >
        ...
        <ScrollView style={styles.scrollView}>
          {/* 현재 레벨 카드 */}
          <View
            style={[styles.levelCard, { backgroundColor: currentLevelColor }]}
          >
            ...
            <View style={styles.progressTextContainer}>
              <Text style={styles.progressText}>
                다음 레벨까지: {levelProgress.current.toLocaleString()} /{" "}
                {levelProgress.required.toLocaleString()} XP
              </Text>
                ...
                <View style={styles.nextLevelItemContent}>
                  ...
                  {nextLevelBadges.map((badge) => (
                    <View key={badge.id} style={styles.nextLevelBadge}>
                      <Text style={styles.nextLevelBadgeIcon}>
                        {badge.icon}
                      </Text>
            ...
            {nextLevelFeatures.length > 0 &&
              nextLevelFeatures.map((feature, index) => (
                <View key={index} style={styles.nextLevelItem}>
                  ...
                  <View style={styles.nextLevelItemContent}>
                    <Text style={styles.nextLevelItemTitle}>
                      {feature.name}
                    </Text>
            ...
            {nextLevelBadges.length === 0 && nextLevelFeatures.length === 0 && (
              <View style={styles.emptyNextLevel}>
                <Text style={styles.emptyNextLevelText}>
                  Lv.{level + 1}에서는 특별한 혜택이 없습니다. 계속해서 일정을
                  완료하고 포인트를 모아보세요!
                </Text>
              </View>
            )}
          ...
          <View style={styles.section}>
            ...
            <View style={styles.xpTableContainer}>
              ...
              {Array.from({ length: Math.min(10, level + 5) }, (_, i) => {
                ...
                const cumulativeXP = Array.from({ length: lvl }, (_, j) =>
                  getRequiredXP(j + 1)
          ...
          <View style={styles.section}>
            ...
            <View style={styles.tipItem}>
              <Text style={styles.tipIcon}>✨</Text>
              <Text style={styles.tipText}>
                하루의 모든 일정을 완료하여 '완벽한 하루' 보너스를 받으세요
              </Text>
            </View>
          </View>
          {/* 하단 여백 */}
